...
export default function Cart() {
  ...
  const checkout = async () => {
    ...
    const res = await api.post("/cart/checkout", { shippingAddress });
        ...
        <div id="cart-content">
          {cart.items.map((i) => (
            <div className="cart-item" key={i._id}>
              <img
                ...
                onError={(e)=>e.currentTarget.src="https://images.unsplash.com/photo-1560472354-b33ff0c44a43?w=80&h=80&fit=crop"}
              />
              <div className="cart-item-details">
                <h3 className="cart-item-title">{i.product?.title}</h3>
                <p className="cart-item-price">${Number(i.product?.price || 0).toFixed(2)}</p>
                {typeof i.product?.co2Saved === "number" && (
                  <p style={{ color: "var(--muted-foreground)", fontSize: ".875rem" }}>
                    CO₂ saved: {i.product.co2Saved.toFixed(1)} kg
                  </p>
                )}
                <div style={{ marginTop: ".5rem" }}>
                  <button className="view-button" onClick={()=>updateQty(i._id, i.quantity - 1)}>-</button>
                  <span style={{ margin: "0 .75rem" }}>{i.quantity}</span>
                  <button className="view-button" onClick={()=>updateQty(i._id, i.quantity + 1)}>+</button>
                </div>
              </div>
              <div className="cart-item-actions">
                <button className="remove-button" onClick={()=>removeItem(i._id)}>Remove</button>
              </div>
            </div>
          ))}
          ...
          <div className="cart-summary">
            ...
            {typeof cart.totalCO2Saved === "number" && (
              <p style={{ color: "var(--primary)", fontWeight: 500, marginBottom: "1rem", textAlign: "center" }}>
                🌱 You'll save {cart.totalCO2Saved.toFixed(1)} kg of CO₂ with this purchase!
              </p>
            )}
